<script lang="ts">
  ...
  // Get the proper image URL - ensure it's absolute
  $: imageUrl = message.message_type === 'image' ? getImageUrl(message.content) : '';
  $: isValidImageUrl = message.message_type === 'image' && message.content !== 'www.chatwii.com' && !message.content.includes('www.chatwii.com');

  function getImageUrl(url: string): string {
    // Skip invalid URLs (corrupted by old link blocking)
    if (url === 'www.chatwii.com' || url.includes('www.chatwii.com')) {
      return '';
    }
    ...
    // If URL is relative or just a path, prepend ImageKit endpoint
    const urlEndpoint = (typeof window !== 'undefined' && window.ENV?.PUBLIC_IMAGEKIT_URL_ENDPOINT) ||
    ...
    >
      {#if message.message_type === 'image'}
        ...
        {:else}
          ...
          <div class="w-[200px] h-[200px] sm:w-[300px] sm:h-[300px] bg-neutral-100 dark:bg-neutral-800 rounded-xl flex items-center justify-center p-4">
            <div class="text-center">
              <svg class="w-12 h-12 mx-auto mb-2 text-neutral-400 dark:text-neutral-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
              </svg>
              <p class="text-xs text-neutral-500 dark:text-neutral-400">Image unavailable</p>
            </div>
          </div>
        {/if}
      {:else}
